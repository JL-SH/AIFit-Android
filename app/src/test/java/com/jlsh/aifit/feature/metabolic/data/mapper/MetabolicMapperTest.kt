...
class MetabolicMapperTest {
    ...
    fun `toDomain mapea MetabolicAnalysisResponseDto correctamente`() {
        ...
        val weightTrend = requireNotNull(result.weightTrend)
        assertEquals(-0.3, weightTrend.averageWeeklyChange, 0.001)
        assertEquals("LOSING", weightTrend.trend)
        assertEquals(-0.5, weightTrend.expectedWeeklyChange, 0.001)
        assertEquals(0.2, weightTrend.deviationFromExpected, 0.001)
        assertEquals(14, weightTrend.dataPoints)
