...
class GetWhyThisExerciseUseCaseTest {

    private val repository: EducationRepository = mockk()
    private val useCase = GetWhyThisExerciseUseCase(repository)

    @Test
    fun `cuando repo retorna Success, useCase retorna Success`() = runTest {
        val explanation = fakeWhyThisExplanation()
        coEvery { repository.getWhyThisExercise(any()) } returns Result.Success(explanation)
    ...
    fun `cuando repo retorna Error, useCase retorna Error`() = runTest {
        coEvery { repository.getWhyThisExercise(any()) } returns
